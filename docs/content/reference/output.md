...
describes what users get. Pick a format with `-o`, or let qqmusic choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
qqmusic <command> -o table   # aligned columns for reading
qqmusic <command> -o jsonl   # one JSON object per line, for piping
qqmusic <command> -o json    # a single JSON array
qqmusic <command> -o csv     # spreadsheet friendly
qqmusic <command> -o tsv     # tab-separated
qqmusic <command> -o url     # just the URL column
qqmusic <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
qqmusic <command> --fields id,title,url
```
...
```bash
qqmusic <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
qqmusic <command>            # a table, because this is a terminal
qqmusic <command> | wc -l    # JSONL, because this is a pipe
```
